...
function renderTables() {
    ...
    tables.forEach(table => {
        const div = document.createElement('div');
        div.className = `table ${table.is_available ? 'available' : 'unavailable'}`;
...
function createOrder() {
    console.log('Creating order:', orderItems);
...
// function createOrder() {
//     const token = localStorage.getItem('access_token');
//     fetch('http://localhost:8000/orders/', { // Adjust URL to your actual endpoint
//         method: 'POST',
//         headers: {
//             'Authorization': `Bearer ${token}`,
